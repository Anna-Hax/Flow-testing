...
import SimpleNFT from 0x0095f13a82f1a835
  ...
  transaction(
      ...
      prepare(signer: auth(BorrowValue) &Account) {
          ...
          self.minter = signer.storage.borrow<&SimpleNFT.NFTMinter>(from: SimpleNFT.MinterStoragePath)
              ?? panic("The signer does not store a SimpleNFT Collection object at the path "
                          .concat(SimpleNFT.CollectionStoragePath.toString())
          ...
          self.recipientCollectionRef = getAccount(recipient).capabilities.borrow<&{NonFungibleToken.Receiver}>(
                  ...
                  .concat(SimpleNFT.CollectionPublicPath.toString())
