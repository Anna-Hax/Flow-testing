...
access(all) contract SimpleScheduledMarketplace {
    // Events
    ...
    access(all) event FundsTransferred(funds : String)
    access(all) event RandomEvent(eve : String)
    access(all) event Randomevent2(eve : String)
    access(all) event Randomevent3(eve: String)
    access(all) event Randomevent4(eve: String)
    access(all) event Randomevent5(eve: String)
    access(all) event Randomevent6(eve: String)
    access(all) event Randomevent7(eve: String)
    access(all) event Randomevent8(eve: String)
    access(all) event Randomevent9(eve: String)
    access(all) event Randomevent10(eve: String)
    ...
    init(feePercent: UFix64) {
        ...
        self.bidVaults <- {}
        self.saleCollection <- SimpleNFT.createEmptyCollection(nftType: Type<@{NonFungibleToken.NFT}>())
    }
    //
    // LIST & BID
    //
    ...
    // CANCEL
    //
    access(all) fun cancelListing(listingID: UInt64, seller: Address, sellerCollection: &SimpleNFT.Collection) {
    ...
    access(all) fun completeAuction(listingID: UInt64) {
        ...
        if listing.highestBidder == nil || listing.currentBid == listing.basePrice {
            ...
            let sellerCollection =getAccount(listing.seller).capabilities.borrow<&{NonFungibleToken.Receiver}>(
                ...
                ?? panic("Seller has not published a public SimpleNFT collection capability")
            sellerCollection.deposit(token: <- nft)
        ...
        log("1. winnningVault balance: ".concat(winningVault.balance.toString()))
        emit RandomEvent(eve: "1. winnningVault balance: ".concat(winningVault.balance.toString()))
        let fee: UFix64 = finalAmount * self.feePercent
        log("2. fee: ".concat(fee.toString()));
        emit Randomevent2(eve: "2. fee: ".concat(fee.toString()))
        let sellerAmount: UFix64 = finalAmount - fee
        log("3. sellerAmount: ".concat(sellerAmount.toString()));
        emit Randomevent3(eve: "3. sellerAmount: ".concat(sellerAmount.toString()))
        ...
        log("4. ownerReceiver: ".concat(self.owner.toString()));
        emit Randomevent4(eve: "4. ownerReceiver: ".concat(self.owner.toString()))
        ...
        log("5. sellerReceiver: ".concat(listing.seller.toString()));
        emit Randomevent5(eve: "5. sellerReceiver: ".concat(listing.seller.toString()))
        if fee > 0.0 {
            let feeVault <- winningVault.withdraw(amount: fee)
            ownerReceiver.deposit(from: <- feeVault)
            log("Deposited fee of ".concat(fee.toString()).concat(" to owner ").concat(self.owner.toString()))
            emit Randomevent6(eve: "Deposited fee of ".concat(fee.toString()).concat(" to owner ").concat(self.owner.toString()))
        }
        if sellerAmount > 0.0 {
            let sellerVault <- winningVault.withdraw(amount: sellerAmount)
            sellerReceiver.deposit(from: <- sellerVault)
            log("Deposited ".concat(sellerAmount.toString()).concat(" to seller ").concat(listing.seller.toString()))
            emit Randomevent7(eve: "Deposited ".concat(sellerAmount.toString()).concat(" to seller ").concat(listing.seller.toString()))
        }
        destroy winningVault
        // transfer NFT to winner
        let nft <- self.saleCollection.withdraw(withdrawID: listing.tokenID)
        log("Withdrew NFT ID ".concat(nft.id.toString()).concat(" from saleCollection"))
        emit Randomevent8(eve: "Withdrew NFT ID ".concat(nft.id.toString()).concat(" from saleCollection"))
        let winner = listing.highestBidder!
        log("Winner is ".concat(winner.toString()))
        emit Randomevent9(eve: "Winner is ".concat(winner.toString()))
        let winnerCollection = getAccount(winner).capabilities.borrow<&{NonFungibleToken.Receiver}>(
                  SimpleNFT.CollectionPublicPath
          )
            ?? panic("Winner has not published a public SimpleNFT collection capability")
        log("Depositing NFT to winner's collection")
        emit Randomevent10(eve: "Depositing NFT to winner's collection")
        winnerCollection.deposit(token: <- nft)
        log("Deposited NFT ID to winner ")
